FROM debian
RUN apt update
RUN DEBIAN_FRONTEND=noninteractive apt install firefox-esr mate-system-monitor  git lxde tightvncserver wget vim openssh-service   -y
RUN wget https://github.com/novnc/noVNC/archive/refs/tags/v1.3.0.tar.gz
RUN tar -xvf v1.3.0.tar.gz
RUN mkdir  /root/.vnc
RUN echo 'asdf1234' | vncpasswd -f > /root/.vnc/passwd
